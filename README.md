...
- **Live application**: [`slope-beta.vercel.app`](https://slope-beta.vercel.app) — create a schedule from the browser (email onboarding, no seed phrase) and watch delegated execution in real time.
...
- **The keeper consumes this live subgraph data to decide its work**: every tick queries the API-key-authenticated versioned Studio endpoint for its execution candidates — which active positions to run, the indexed `executedAmount` as the due-increment basis (not-due positions cost no RPC), and skip history for parking and diagnosis. There is deliberately no local-data fallback: if the subgraph is unreachable, the tick is skipped and the reason logged. The boundary is explicit (SPEC section 5): indexed data chooses candidates, but the position's authoritative state, price, quotes, and slippage are read on-chain at execution time — the subgraph's indexing lag must never decide a financial outcome.
...
- [`docs/DEMO_VIDEO_SCRIPT.md`](docs/DEMO_VIDEO_SCRIPT.md) — the submission recording sequence.
...
- **The Graph** — a Subgraph Studio deployment on Base Sepolia indexing positions, fills, skips, and the benchmark snapshot; the keeper consumes this live, API-key-authenticated data from the pinned versioned endpoint to select and rank execution candidates and to park problem positions from indexed skip history, while execution-critical state stays on-chain; the dashboard benchmarks realized execution against the linear-TWAP baseline.

## Known Limits And Roadmap

Three boundaries we understand, each with a path out.

**Time-driven by design — a price-driven mode is the next model.** Slope's schedules are currently time-driven: every schedule has a duration, the curve sets how fast the budget may execute inside it, and the terminal clamp guarantees whatever remains can always settle once the window closes. Price rails hold fills back while conditions are bad, but the schedule — not the market — decides when execution is allowed. The inverted model is a real alternative: price-driven schedules with no duration at all. The user sets an acceptable price range and the system executes the next slice whenever the market enters it, waiting indefinitely if it never does. That removes any pressure to finish by a deadline, and the trade-offs are why we did not start there: an order with no time bound can hang forever if price never returns to the requested range, leaving the user's commitment unresolved until manual cancellation; and a system that only waits for good prices converges toward an ordinary limit order, while Slope's core value is pacing — reducing the price impact the order itself creates. The natural destination is a hybrid, where duration becomes optional rather than mandatory: a schedule without one waits on price conditions for as long as it takes, a schedule with one behaves exactly as shipped here. That change touches the contract, so it sits outside this hackathon's scope. One practical note in the current model's favor: it demos itself — a fifteen-minute schedule completes in front of the audience, while a price-waiting schedule may legitimately do nothing on camera.

**Privy spend aggregation is a 72-hour rolling window, not a lifetime cap.** The delegated session signer's cumulative-spend tracking is evaluated over rolling windows of at most 72 hours, and denied attempts count against the window. Per-schedule budget enforcement therefore lives where it is airtight — the contract's `executedAmount <= totalBudget` invariant and the per-transaction cap in the signing policy — while the rolling window acts as a blast-radius bound on the signer. A hard lifetime budget per session key would need a longer-horizon aggregate, tracked off-chain by the keeper or enforced through scheduled policy expiry, both of which the current architecture already anticipates.

**External wallets work through a smart account.** Session-signer delegation controls Privy-managed embedded wallets, so external wallets (MetaMask and friends) can browse the interface but cannot delegate execution. The way out is structural rather than speculative: `SlopePosition` is permissionless, so any account capable of holding a policy-bound session signer works — an external EOA wrapped in a smart account with the same scoped-signer policy delegates exactly as an embedded wallet does. Not built for the hackathon; not blocked by anything in the design.
