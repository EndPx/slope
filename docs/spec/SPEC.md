...
2. **Probe floor derived from `decimalsIn`, with an honest impact-check limit.** The floored probe (`fillAmount / 1000`) could collapse to 1 wei on 6-decimal tokens; a 1-wei quote plausibly returns zero output, stranding positions behind a misleading skip. The probe is now `max(fillAmount / 1000, 10^(decimalsIn - 4))`, capped at the fill itself — 1/10000 of a whole tokenIn unit (100 raw USDC; 0.0001 WETH), chosen so the floor stays below ordinary fills on both decimal shapes. **Honest limit**: when `fillAmount < probeFloor` the fill becomes its own probe — `referencePrice == executionPrice`, the impact measurement cannot measure anything and is **not applied**; `FillExecuted` carries `impactChecked = false` so the Subgraph and dashboard never present an unchecked fill as verified, and the absolute `[minPrice, maxPrice]` bounds remain the only price protection for those fills. No document or README may claim every fill is price-impact checked. The floor is validated against the test mocks for 18- and 6-decimal `tokenIn`, and must be re-validated against the real router at step 3.
...
**OPEN ITEMS — CLOSED at step 3 fork validation (2026-09-05, Base mainnet fork against the official router `…c0de`; contracts/test/AquaBaseFork.t.sol):**
- **OI-1 — exact-in consumption semantics: VERIFIED.** The official router consumes exactly the requested input: the registry's virtual tokenIn balance delta equals `fillAmount` and the taker wallet delta equals `fillAmount` (test_Fork_ExactInConsumesFullInputOnRealRouter). `executedAmount += fillAmount` stands unchanged.
- **OI-2 — swap-level minimum-output threshold: MECHANISM VERIFIED, WIRING RULE SET.** A `threshold` carried in `takerTraitsAndData` (32-byte minimum output for exact-in) is enforced by the swap itself: a threshold above the achievable output reverts the swap (test_Fork_ThresholdAboveQuote_RevertsAtSwapLevel). Rule: route blobs MUST embed a threshold — test blobs use the quoted output as min-out; production route updates embed the pre-swap quoted output as the hard floor.
- **Probe floor validated on the real router (REVISION 3 follow-up)**: `10^(decimalsIn - 4)` notionals return non-zero quotes for both 18- and 6-decimal tokens against seeded xycSwap liquidity (test_Fork_ProbeFloorReturnsMeaningfulQuotesOnRealRouter). No floor adjustment needed.
...
**VERIFIED IMPLEMENTATION NOTES (2026-09-06, live delegation debugging on Base Sepolia)**:
- **Privy aggregation-reference conditions deny every `eth_signTransaction` they appear in.** Bisected on a live policy: identical policies with the reference (operators `lte`/`lt`, values hex/decimal, fresh and reused windows) were always denied `policy_violation`; the same policy without it signed, broadcast, and filled on-chain. This — not aggregation exhaustion — was the cause of every earlier denial. Per-position policies therefore enforce: `to` allowlist + `adaptiveExecute` selector + **own-position binding** (`adaptiveExecute.positionId` eq) + per-tx `maxAmountIn` cap + expiry; the rolling-sum rate limit moved to the keeper (it signs only the curve-authorized increment) with the on-chain `executedAmount <= totalBudget` invariant as the hard bound. The per-delegation aggregation was dropped (the 10-per-app slot limit becomes moot for new delegations; DELETE /v1/aggregations recycles existing ones; no collection GET exists — HTTP 405 — and the recorded sum is not API-readable).
- **Decoded-calldata comparisons use DECIMAL strings and `lte` is INCLUSIVE at the boundary**: a terminal settlement signing exactly `totalBudget` passes a cap of exactly `totalBudget` (live tx). Hex-formatted cap values are unreliable.
- **`eth_signTransaction` signs params as given**: nonce, `gas_limit`, and EIP-1559 fees must be supplied by the caller or the raw tx fails at broadcast with "intrinsic gas too low".
- **A successful transaction is not a successful fill**: the skip-not-revert design emits `PositionSkipped` with status success. The keeper decodes the receipt and treats a skip as no progress; `TRANSFER_FAILED` (owner wallet out of tokenIn balance/allowance) parks immediately — only an owner-side fund+approve can change it, and the delegated signer is deliberately scoped out of both.
- **Positions sharing a wallet share one nonce sequence**: parallel per-position workers must serialize per owner wallet, or concurrent fills reject each other with "replacement transaction underpriced" (self-healed by retry, but wasteful).
- **Measured sign-ledger inflation (validates the rate-limit design):** Σ of signed `maxAmountIn` exceeded 1× budget under failure patterns — 179.98% for a position that needed one broadcast-failure retry, 1500%+ during the pre-fix skip loop. A policy-level rolling sum cap tight enough to be meaningful (2× budget) would have bricked both positions; this empirically confirms the rate limit belongs in the keeper + on-chain invariant, not in a Privy rolling sum.
